...
export function FormulaPage({ routeName }: FormulaPageProps) {
  ...
  // scrollspy 回调：仅更新高亮，绝不触发滚动（否则会覆盖用户点击的目标分类）
  const onHighlight = useCallback((cId: string) => {
    setActiveCat(cId);
  }, []);
  ...
  return (
    <div className="fields-root">
      <PageHeader
        sticky
        title={favMode ? '★ 收藏公式' : view.title}
        subtitle={
          favMode ? `跨科目 · 共 ${favList.length} 式` : view.subtitle
        }
      >
        <button
          type="button"
