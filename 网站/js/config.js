/* ==========================================================================
 * config.js — 大纲目录数据（单一数据源）
 * 其他模块一律从此 import，禁止在他处硬编码章节信息。
 * ========================================================================== */

/**
 * 题型 -> { label: 中文标签, type: 徽标语义键(对应 CSS .badge[data-type]) }
 */
 ...
 * @property {string} id        - 章节序号 "01".."10"，用于路由
 * @property {string} title     - 章节名（不含"第N章"前缀）
 * @property {string} file      - 相对 网站/index.html 的 md 文件路径
 * @property {{key:string,range:string}[]} topics - 题型分布
 * @property {number} importance - 1~3 星
 *
 * @typedef {Object} Subject
 * @property {string} id     - "shudian" | "modian"
 * @property {string} name   - 科目中文名
 * @property {string} short  - 短称（顶栏 tab）
 * @property {string} dir    - md 文件所在目录（相对 index.html）
...
/** 默认路由（首屏）*/
export const DEFAULT_ROUTE = { subjectId: 'shudian', chapterId: '01' };
